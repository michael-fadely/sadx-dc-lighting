...
#define STORE_RS(RS) \
	DWORD RS; \
	d3d::device->GetRenderState(D3DRS_ ## RS, &RS)  // NOLINT(bugprone-macro-parentheses)

#define RESTORE_RS(RS) \
	d3d::device->SetRenderState(D3DRS_ ## RS, RS)

template <class T>
inline void hash_combine(std::size_t& seed, const T& v)  // NOLINT(clang-diagnostic-shadow)
{
	std::hash<T> hasher;
...
struct OhKey
{
	const ObjectMaster* master;
	const NJS_OBJECT* owner;
	const NJS_MODEL_SADX* model;

	OhKey(const ObjectMaster* master, const NJS_OBJECT* owner, const NJS_MODEL_SADX* model)
		: master(master),
		  owner(owner),
		  model(model)
	{
...
template <>
struct std::hash<OhKey>
{
	size_t operator()(const OhKey& x) const noexcept
	{
		size_t h = std::hash<const ObjectMaster*>()(x.master);
...
#pragma pack(push, 1)
	struct QuadVertex
	{
		static constexpr UINT format = D3DFVF_XYZRHW | D3DFVF_TEX1;
...
#pragma pack(pop)
	...
	static void draw_quad()
	{
		...
		const float width_half_pixel  = static_cast<float>(present.BackBufferWidth) - 0.5f;
		const float height_half_pixel = static_cast<float>(present.BackBufferHeight) - 0.5f;
	...
	static void create_render_targets()
	{
		...
		HRESULT hr = device->CreateTexture(HorizontalResolution, VerticalResolution, 1, D3DUSAGE_RENDERTARGET,
...
#pragma region Trampolines
	...
	static void store_transform(ObjectMaster* master, NJS_OBJECT* owner, NJS_MODEL_SADX* model)
	{
		...
		const OhKey key(master, owner, model);

		const auto world_matrix = *reinterpret_cast<D3DXMATRIX*>(_nj_current_matrix_ptr_) * _view_matrix_inv;

		const auto it = transform_map.find(key);
		...
		if (it != transform_map.end())
		{
			...
			it->second.world = world_matrix;
			it->second.view = _view_matrix;
			it->second.proj = _proj_matrix;
		}
		else
		{
			transform_map[key] = { world_matrix, _view_matrix, _proj_matrix };
			param::l_WorldMatrix = world_matrix;
			...
			if (v2)
			{
				do
				{
					...
					*(void**)0x3ABDBF4 = mainsub;
					if (mainsub)
					{
						CurrentObject = reinterpret_cast<ObjectMaster*>(v2->field_30);
						mainsub(v2);
	...
	void __cdecl RunObjectChildren_r(ObjectMaster *a1)
	{
		...
		if (child)
		{
			do
			{
				...
				CurrentObject = reinterpret_cast<ObjectMaster*>(child->field_30);
				child->MainSub(child);
				CurrentObject = nullptr;
	...
	void __cdecl DisplayObjectChildren_r(ObjectMaster *this_)
	{
		ObjectMaster *v1; // esi
...
namespace d3d
{
	...
	void init_trampolines()
	{
		...
		DisplayObjectChildren_t            = new Trampoline(0x0040B130, 0x0040B135, DisplayObjectChildren_r);
